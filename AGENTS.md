...
5. Version Management:
   - Chrome拡張機能パッケージ（`projects/app/` 配下）に含まれるファイルに修正がある場合、必ずバージョンを更新すること。
   - 機能拡張や改善、UI変更を含む場合はマイナーバージョンをインクリメント（例: 0.1.x -> 0.2.0）する。
   - 不具合修正のみの場合はパッチバージョンをインクリメント（例: 0.1.0 -> 0.1.1）する。
   - ドキュメント（`README.md`, `AGENTS.md` 等）、GitHub Actions ワークフロー、メンテナンススクリプトのみの修正で、拡張機能本体に影響がない場合はバージョン更新を行わない。
   - バージョンを更新する際は、`manifest.json`, `package.json`, `package-lock.json`, `README.md` のバッジのすべてで整合性を保つこと。
